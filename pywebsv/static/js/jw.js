...
(function($){
	...
	over:function(opt){
		opt=opt||'show';
		var over,bh=document.body.scrollWidth;//$('body').height();
		if(opt=='show'){
			...
			$().ready(function(){
				...
				(typeof $.fn.bgiframe=='function') && over.bgiframe();//alert(bh)
				over.height(bh);
			});
		}else{
...
//	          	     });
	        ...
	        $(win).resize(fn);
	        dialog.find('.close').bind('click mousedown',close);
