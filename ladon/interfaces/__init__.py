...
def expose(class_type):
	...
	if inspect.getmro(class_type).count(BaseInterface):
		if_name = class_type._interface_name()
		if type(if_name)!=str:
			raise ExposeInterfaceException(class_type.__name__,"wefew",'Interfaces must implement the static _interface_name method to return a unique string identifying the interface.')
